...
public class DebugInfoItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if (player.level().isClientSide) { // Most data is always available on the client, even if the user isn't in VR
            player.sendSystemMessage(Component.translatable("item.vr_iron_man.debug_info.client_title").withStyle(ChatFormatting.UNDERLINE));
            ...
            if (data != null) { // If it's null, we weren't in VR according to the server.
                player.sendSystemMessage(Component.translatable("item.vr_iron_man.debug_info.server_title").withStyle(ChatFormatting.UNDERLINE));
                ...
                player.sendSystemMessage(Component.translatable("item.vr_iron_man.debug_info.not_in_vr"));
            }
        }
